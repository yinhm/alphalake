...
财务导出与准入已升级到v2，业务取数与审核政策使用通用字段名，财务库要求schema46。源编号保留在血缘，当前只接受v2；所需旧输入已一次性转换，历史复验使用原提交，不保留运行时转换器或别名。详见[标准消费分层](../../docs/decisions/018-standard-financial-consumption.md)。
