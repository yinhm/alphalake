...
调用链 `orchestrator` → `module_1_adjustments` → `module_3_cashflow` 已检查：研发资产及摊销可以计算，但基础ROIC的期初资本目前使用当前研发资产代理；历史序列也复用当前资产。`company_metrics` 的两期资本使用同一当前调整，不能据此识别研发资产增加带来的边际投入。

税后利润方面，M1直接加回“当期研发－摊销”到净利润，M3却给调整后EBIT全额乘税后系数；`company_metrics`及M5利润率亦有此路径。对保持税项不变的分析重分类，这与[已有安克情景](../../../internal/ingest/testdata/anker-history-2026/README.md)不一致。应先统一税项与期初/期末口径，再用结果估计回报；本次未改共享模型，未声称以上路径已修复或已获准确度证明。
