...
后续[安克五年度资本分量衔接](anker-capital-components.md)已完成50项比较（含本轮10项），49匹配、1项版本冲突；随后[苏泊尔三年金融资产分类](supor-capital.md)已核对40个金额及9个源位，确认未分类流动余额不能作经营资本标签；完整分类仍未闭合。需要的是正常经营下新增资本的归属与时序，不是先解释完极端公司。分类不足时，不用行业比率反推真实资本，不将报表现金代理当作FCFF实际值，也不据此宣布估值准确度提高。
