...
def forecast(base,growth,horizon):
    revenue=base['revenue'];margin=base['ebit']/revenue;annual=[]
    for _ in range(horizon):
        revenue*=1+growth*.5;annual.append(dict(revenue=revenue,ebit=revenue*margin,growth=growth*.5))
...
def run(p):
    if p['protocol_id']!='tdx-half-growth-v1' or p['growth_multiplier']!=.5 or (p['baseline'],p['candidate'],p['benchmark'])!=MODELS:raise ValueError('unsupported half growth')
    ...
    for original_row in original['results']:
        ...
        if 'forecasts' in r:
            ...
            r['forecasts'][MODELS[1]]=forecast(r['base'],r['rule_evidence']['clipped_scenario_growth'],r['horizon'])
