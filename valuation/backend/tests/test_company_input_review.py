...
def test_company_input_review(code, tmp_path, monkeypatch):
    ...
    request = AlphaLakeRequest.model_validate(json.loads(gzip.decompress((ROOT/f'{code}-request.json.gz').read_bytes())))
...
def test_anker_asset_amounts_against_existing_pdf_ledger():
    ...
    import hashlib
    import struct
    evidence = Path(__file__).resolve().parents[3]/'internal/ingest/testdata/anker-valuation-2026'
    request = AlphaLakeRequest.model_validate(json.loads(gzip.decompress((ROOT/'300866-request.json.gz').read_bytes())))
...
def test_anker_lease_additions_tamper_rejected(tmp_path, monkeypatch):
    ...
    current = Path('valuation/research/current-contract-20260919/company-inputs-20260917')
    shutil.copytree(module.ROOT/current, tmp_path/current)
