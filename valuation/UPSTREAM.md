...
合仓路径迁移、根 CI 后端/双公司验收及 AlphaLake 数据库到原生估值桥接已完成。旧适配实验的 `target_origin_revision` 仅记录来源版本；当前代码由本仓库管理。新增导出契约、审核附注导入、HTTP 入口与股权桥接的使用和边界见 [融合说明](docs/alphalake-integration.md)。这些属于 AlphaLake 后续开发，上述原作者与许可证保留。
