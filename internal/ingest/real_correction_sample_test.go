...
func TestRealCorrectionWithoutOriginalProviderVersion(t *testing.T) {
	...
	check(err)
	if result.Linked != 1 || result.Inserted != 44 || result.Rejected != 7 || result.LinkAmbiguous != 0 {
	...
	check(err)
	if replay.Inserted != 0 || replay.Updated != 0 || replay.Removed != 0 || replay.Materialized != 44 {
