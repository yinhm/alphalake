...
func TestRealFinancialWorkflow(t *testing.T) {
	...
	check(err)
	if materialized.Inserted != 269 || materialized.Linked != 6 || materialized.Rejected != 1 || materialized.LinkPending != 0 || materialized.LinkAmbiguous != 0 {
	...
	count("SELECT count(*) FROM fundamental.fact WHERE period_type='FY' AND unit='CNY'", 83)
	...
	count("SELECT count(*) FROM fundamental.fact_asof(TIMESTAMPTZ '2026-03-06 16:00:00+00')", 269)
	...
	count("SELECT count(*) FROM fundamental.fact", 269)
	t.Logf("真实样本验收通过：公告=12，目录页=3，正文=%d，源事实=3504，标准事实=269，独立金额=16；失败恢复、幂等及 PIT 边界通过", len(documents))
}
