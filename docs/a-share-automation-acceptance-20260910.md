...
刷新命令默认在同步结束后选择信息时点，不固定到本次验收日期；依次采集、补公告、物化、同步参考、批量计算。现已[接入增量重估](../valuation/research/automatic-valuation-20260917/README.md)：同一output-dir原子保留incremental-state.json，当前校验通过后才复用未变输入；变化重新计算并比较，失败仅保留last_success历史证据，不输出旧值作本次价格。运行日志/退出码与批次覆盖分别检查，部分失败不能当全量刷新成功。内存和六小时阶段上限为本机部署参数，不是资源或完成时长保证。已有入口可交cron/systemd调用，未安装后台周期任务。
