...
公司行业归属、行业权重、国家风险暴露权重及 lambda、期限、是否扣主权违约利差、目标债务权重、税盾率、税前借款成本都来自附理由的政策。默认不使用行业 D/E 替代公司目标结构；仅显式行业参考目标政策可采用D/E代理（见下文）。行业有效税率不替代公司边际税率。权重须各自合计为 1；国家风险贡献允许通过 exposure_scale 单独缩放。
...
后续已接入[合成评级利差／借款成本估计](credit-spread-sync.md)：可选第四份发布版本及安克合成借款成本政策，原显式政策仍兼容。

## 通用非金融政策

`nonfinancial-earnings-power-v1`、`nonfinancial-book-fcff-v1`、`nonfinancial-history-fcff-v1` 同样支持上述 CLI／HTTP 参考绑定。删除经营政策顶层 `wacc`（不同于专用模型的 `parameters.wacc`），提供实际证券代码、合并范围、期间和有理由的 WACC 政策。目标权重、行业与国家暴露仍须显式审核，不能将安克示例参数直接认作其他公司的事实。计算后重新检查 WACC 大于终值增长，并在结果中记录选中参考、政策及血缘。

通用模型当前仅支持目标资本权重；类别股本、经营权益范围和债务市值桥接尚未通用化，市场资本绑定会被拒绝。行业自动路由仍使用各规则的固定 WACC，参考绑定可先用于逐公司 assignment；未宣称已自动生成全市场 WACC 政策。

## 自动选择本地参考版本

日常运行可以用 `alphalake export-wacc-references DB --as-of RFC3339 --latest`，无需手填四个 release_id。它按每类数据的观察日期降序、同观察日期修订的入库时间降序选择，要求国家风险、行业Beta、国债曲线、信用利差四类均存在；与任何显式 release 参数互斥。公开可用时点和可选 `--recorded-cutoff` 同时约束选择，未来观察不参与。迟到的旧日期不覆盖新日期，同日期/入库时间并列则拒绝，不以ID强行打破歧义。

所选版本再走原固定版本完整性/检查点/血缘校验；所选版本损坏或缺行时失败，不回退到旧版本。输出依然保存具体 release_id、内容哈希及方法，固定ID模式仍用于精确重放。自动选择不保证数据足够新，估值政策的陈旧天数继续独立检查，也不自动同步外部数据。

真实参考库 `workspace/wacc-beta-yield-20260909/acceptance.duckdb` 在2026-09-10T01:24:27Z自动选出Beta1、国债2、国家风险3、信用4，输出与同一时点的手工固定四版本包完全相等。回归同时覆盖迟到旧观察、损坏新修订、选择并列、时点边界与缺失曲线。

## 行业参考目标权重

新增显式 `capital_structure_basis=industry_reference_weights` 政策，可直接消费同一版本Beta表的 `debt_equity_ratio`。它禁止同时填写 `target_debt_weight` 或市场资本政策，不影响原直接目标权重和市场权益/估计债务分支。

先计算 `行业D/E = Σ(政策行业权重 × 来源行业D/E)`，再取 `债务权重 = 行业D/E / (1 + 行业D/E)`；多行业时是加权比率代理，不声称将各行业公司资本金额重新汇总。按该目标结构对行业Beta加杠杆，税盾仍使用显式政策。D/E须非负、方法为provider_reported、样本数与选中Beta一致，并通过既有日期、单位、唯一性及发布血缘门槛。

结果保留选中的D/E观测和 `industry_capital` 审计，状态为 `industry_target_proxy_not_company_market_structure`，公司市场权益/债务金额保持空值。此功能减少手抄行业比率，不把行业结构冒充公司实际资本结构，也不把行业有效税率改作公司边际税率。

`valuation/examples/a-share-damodaran-wacc-2026H1.json` 在五行业政策上显式启用该分支：行业Beta和D/E、人民币国债、国家风险与评级利差取已发布参考；统一BBB信用档、中国风险暴露、25%税盾及终期保持WACC仍为分析假设。不是公司评级判断，也不证明全市场公司特定WACC已经闭合。该版本与固定10%政策分别保留，可在相同财务/信息时点对比。

该分支已通过真实财务/参考包的双行业Decimal独立WACC复算与API重放，覆盖禁止手填权重、负D/E、样本不一致、单位错标，以及既有错证券/范围/过期参考拒绝。Python全套226通过、4项既有跳过；Go全套、构建和vet通过，无依赖变化。跨公司参考WACC批次结果另行验收，不能从此测试直接推断全市场适用。
