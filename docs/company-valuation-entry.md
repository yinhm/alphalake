...
[两份真实运行验收](acceptance/forecast-review-20260911.json)：安克/苏泊尔2026H1请求均完整重放，分别保留10个未到期位置；将导出可执行文件设为`/bin/false`仍通过，证明未提前查询。主运行目录核查的3,117条记录均为2026H1，尚无一年到期结果；这不是3,117家公司计数。到期数值分支使用人工旧预测配合Go标准财务导出夹具检验，不冒充已存在的真实2025运行或准确度实证。
