...
不确定性证据接入已完成[两份安克主库前后对照](acceptance/cash-uncertainty-output-20260911.json)：通用情景92.1504元、零增长情景42.2846元及现金预测均不变；仅新增证据、检查代码哈希与`check_id`变化。零增长当前ID与更早归档的差异来自本次之前的政策说明更新，数值输入未变。此前并发验收发现的读写ATTACH锁冲突现已通过[只读查询复验](acceptance/readonly-valuation-20260911.json)修复：两路主库CLI并发成功，完整JSON与此前串行结果一致。估值、就绪度、WACC参考、行业资本、行情、市值及status七条查询路径只读打开已有数据库，不自动迁移或创建文件；同步与导入保留写入。此能力限于多个读进程，不承诺跨进程写入与读取并发；底层stderr的结构化转交仍在[待办](review-minor-backlog.md)。本次接入验收：专项3项及Python全套368项通过，4项既有外部数据缺失跳过、7条既有警告；Go全套、构建、143个相关文档链接与五份CLI文件哈希检查通过。证据对象按调用隔离，返回值修改不污染后续结果。
