...
| Validation persistence | Implemented | Daily structural violations are stored in `meta.validation_result`; good bars, validation evidence and quarantine retry state publish atomically per instrument/run. |
...
| Observed TDX security-master lifecycle | Implemented | Production TDX workflows apply verified SH/SZ/BJ security-master snapshots atomically; missing identifiers are closed only from complete/plausibly-sized snapshots, and current-universe queries return open identifiers only. |
...
| TDX / Shenwan industry hierarchy | Implemented | `incon.dat` + TDX industry assignments build tested TDX 3/5/7 and Shenwan 3/5/7/10/13 hierarchies; `sync-industries <db-path>` applies both through the temporal classification store. |
