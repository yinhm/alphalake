...
| 版本化迁移 | 已实现 | 内嵌迁移是 SQL 的唯一事实来源。仅执行待迁移版本，每个版本独立事务，成功后记录版本。当前结构包含 001–018。 |
...
| 已审核字段映射 | 部分实现 | 已映射 29 个字段：FN230–FN238 及 20 个现金、借款、权益、营运资本和再投资明细；新增字段从 2025 年起有效。其他 FN 字段仍保留为无损证据。见 [ADR 009](decisions/009-core-financial-fields.md)。 |
...
| 标准精度/单位策略 | 已审核字段已实现 | FN230–FN237 为单季度人民币金额，期间标为 Q1/Q2/Q3/Q4；FN238 为报告期末股数。新增存量字段为 `instant`，累计资本开支/折旧摊销为 Q1/H1/9M/FY。用 `DECIMAL(38,10)` 确定性表示 float32 值，不恢复源编码缺失的精度。报表范围保持 `provider_default`。 |
...
| CNINFO 数值提取/校验 | 部分实现 | 已固化 40 个真实金额：四家公司 Q4 的 16 个，以及其中两家公司 Q1–Q3 的 24 个；用同期累计值交叉核对季度口径，检查 float32 编码、期间、单位、原始公告关联与 PIT 边界。另有 20 个新增核心字段的 35 个真实金额、两份完整 PDF 及离线重提取验收；零值折旧摊销因缺失歧义拒绝物化。尚无生产 PDF/XBRL 自动提取流程。见[验收报告](acceptance-20260905.md)及[核心字段样本](../internal/ingest/testdata/core-financial-2025/README.md)。 |
...
本轮已新增 20 个映射，完成现金/权益及部分借款、营运资本、资本开支和折旧摊销明细的真实对账；尚不等于全部十二组输入完整。下一阶段补税项、EBIT 调整、应付债券/租赁等债务明细及完整再投资口径，再补齐年度/TTM 及连续历史覆盖，交付一个普通非金融公司的估值输入表。经营现金流不等于 FCFF，投资现金流净额不等于资本开支。折现率、增长、目标利润率和再投资效率等市场输入及预测假设需单独提供，不冒充财务报表事实。
