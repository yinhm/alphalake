...
| 派生研究/估值/因子 | 部分实现 | 已有年度/TTM 财务窗口；已另有安克单公司回溯验证性 DCF；通用估值产品、因子及回测仍在计划中。 |
...
此处“完成”是端到端验证性估值完成，不是财务数据 100% 完整。经营税现金、历史全分类营运变动等仍缺细分披露；当前股价比较还需同期 WACC、可转债/期权及少数股权公允价值、7 月 H 股发行净额与股数调节。本阶段以明示预测/索偿代理完成模型，不冒充补齐这些事实。下一主线可扩大公司样本，检验这套输入流程能否复用；不把安克个别政策直接写成全市场标准。
