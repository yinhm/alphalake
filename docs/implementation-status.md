...
| 版本化迁移 | 已实现 | 内嵌迁移是 SQL 的唯一事实来源。仅执行待迁移版本，每个版本独立事务，成功后记录版本。当前结构包含 001–019。 |
...
本轮扩充前，其中收入、总股本已有标准映射（2/12，约 17%，仅指输入组映射覆盖）；营业利润为部分基础，尚不能直接视为估值 EBIT；其余组尚未标准化。这个比例不是工程完成度、全市场数据覆盖率或估值准确率。原始包中的数百个 FN 字段已能无损保存，不代表都已审核并可用于估值。尚未统计目标公司×报告期×必需字段的完整率。

本轮已新增 20 个映射，完成现金/权益及部分借款、营运资本、资本开支和折旧摊销明细的真实对账；尚不等于全部十二组输入完整。随后又补入 9 个税项、利息、应付债券、非流动租赁负债与使用权资产折旧字段。下一阶段推进年度/TTM 及连续历史覆盖；估值时仍需 EBIT 非经营项目调整、到期债务附注拆分及完整再投资口径，交付一个普通非金融公司的估值输入表。经营现金流不等于 FCFF，投资现金流净额不等于资本开支。折现率、增长、目标利润率和再投资效率等市场输入及预测假设需单独提供，不冒充财务报表事实。
