...
| CNINFO 数值提取/校验 | 部分实现 | 已固化 40 个真实金额：四家公司 Q4 的 16 个，以及其中两家公司 Q1–Q3 的 24 个；用同期累计值交叉核对季度口径，检查 float32 编码、期间、单位、原始公告关联与 PIT 边界。尚无生产 PDF/XBRL 自动提取流程。见[验收报告](acceptance-20260905.md)。 |
...
目前其中收入、总股本已有标准映射（2/12，约 17%，仅指输入组映射覆盖）；营业利润为部分基础，尚不能直接视为估值 EBIT；其余组尚未标准化。这个比例不是工程完成度、全市场数据覆盖率或估值准确率。原始包中的数百个 FN 字段已能无损保存，不代表都已审核并可用于估值。尚未统计目标公司×报告期×必需字段的完整率。

下一阶段先审核并映射现金、有息债务、账面权益和再投资相关明细，以真实财报对账；再补齐年度/TTM 口径及连续历史覆盖，交付一个普通非金融公司的估值输入表。经营现金流不等于 FCFF，投资现金流净额不等于资本开支。折现率、增长、目标利润率和再投资效率等市场输入及预测假设需单独提供，不冒充财务报表事实。
