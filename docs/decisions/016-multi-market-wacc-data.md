...
状态：部分结构已实现。迁移 027 建立发布版本、归档关联及四类参考观测；尚无采集、发布写入 API 或 WACC 选择器，不代表数据源已经接通。
...
- 尚未建立跨 schema 的观测—归档关联、行业节点外键；写入 API 落地前必须补足发布事务校验。SQL 结构不阻止直接修改已发布行，也不独立证明发布完整性、来源白名单、前序版本同范围、原始单位转换或日期时区边界；不能直接对外提供可信发布接口。
