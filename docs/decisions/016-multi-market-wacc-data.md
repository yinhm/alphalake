...
状态：部分结构已实现。迁移 027 建立发布版本、归档关联及四类参考观测；后续已接入 CN/HK/US 国家风险的限定发布链（见[同步说明](../country-risk-sync.md)），全球行业 Beta 与人民币国债收益率也已接入（见[说明](../beta-yield-sync.md)），[WACC 固定版本选择器](../wacc-valuation-bridge.md)已实现；迁移 028 及[合成评级利差链](../credit-spread-sync.md)扩展大型非金融利差档位，汇率待实现。
...
当前 `ref.company`、`ref.exchange`、交易日历仅有结构；`ref.instrument` 混合证券和上市地点属性。`fundamental.fact` 按证券组织，股本未明确公司总量与股份类别范围，旧日线主键不能保留同来源同日多次修订；迁移 029 已新增[大陆行情兼容观测层](../valuation-quotes.md)保留后续采集版本，仍未建立 listing 模型或恢复升级前历史。因此当前能力不能宣称已经支持多市场公司级估值或市场数据双时态。
