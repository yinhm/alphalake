...
状态：部分结构已实现。迁移 027 建立发布版本、归档关联及四类参考观测；后续已接入 CN/HK/US 国家风险的限定发布链（见[同步说明](../country-risk-sync.md)），其余采集与 WACC 选择器待实现。
...
当前 `ref.company`、`ref.exchange`、交易日历仅有结构；`ref.instrument` 混合证券和上市地点属性。`fundamental.fact` 按证券组织，股本未明确公司总量与股份类别范围，日线主键不能保留同来源同日多次修订。因此当前能力不能宣称已经支持多市场公司级估值或市场数据双时态。
...
- 尚未建立跨 schema 的观测—归档关联、行业节点外键；国家风险写入函数已校验归档身份并在事务内创建关联，其他写入 API 落地前仍须补足相应校验。SQL 结构不阻止直接修改已发布行，也不独立证明发布完整性、来源白名单、前序版本同范围、原始单位转换或日期时区边界；仅完成上述校验的限定写入路径可发布，其他表尚不能直接对外提供可信发布接口。
