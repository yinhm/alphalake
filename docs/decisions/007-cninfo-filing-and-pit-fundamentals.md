...
真实标题「浙江东日股份有限公司2025年第三季度报告（更正）」验证了括号内单独「更正」也表示更正后的完整报告。分类器 v2 同时识别中文/英文括号，避免将其当成普通报告后错误优先关联同日更正公告。已采集窗口须用 `sync-filings --rescan` 重扫相关日期，再运行 `materialize-fundamentals` 更新关联；仅升级分类器不会自动重分类旧行。
...
- FN230–FN237 为单季度金额，归一化为人民币元，`period_type` 为 Q1/Q2/Q3/Q4；
- FN238 为报告期末股本，归一化为股，保留对应报告类型标记。
...
## 实际年报数值验证补充（2026-09-05）

年报包中的 FN230–FN237 表示第四季度，不代表全年累计。例如德赛西威 2025 年报的第四季度营业收入为 10,220,627,011.05 元，FN230 编码为 float32 后是 10,220,626,944 元；全年营业收入为 32,557,178,348.37 元。四家公司的营收、归母净利润、扣非净利润和经营现金流共 16 个值，均与年报分季度表按 float32 编码后的结果逐位一致。

`pit-fundamental-v2` 据此按源字段确定期间，修复此前误用公告报告类型将单季度金额标成 H1/FY 的行为。已有数据库运行 `materialize-fundamentals` 即可原位修复，无需重新下载，也不更改原始财务证据。标准字段名称保持不变，查询时必须同时读取 `period_type`；当前映射不提供半年或全年累计金额。

## 2026-09-06 补充：译本分类与估值消费边界

茅台真实目录同时包含同日中文完整年报和“英文版”年报。旧分类器将两者同列 `full`，按既有规则形成真实歧义，不能在研究样本中删候选绕过。分类器 v3 新增 `translation`：标题明确含“英文版”或“英文译本”时，保留报告类型/期间及原始目录证据，但不作为当前中文结构化数值链路的 PIT 锚点，也不参与更正前序选择。该规则不是对所有外语文本的自动识别，不引入按公告 ID、抓取顺序或猜测语言选优。

迁移 021 对已有 CNINFO 周期报告应用同一明确标题规则。已有数据库升级后执行 `materialize-fundamentals`，由既有协调流程更新关联及失效事实；迁移不修改原始归档或源数值。真实双候选回归和从 v20 升级的回归已覆盖。

估值必须遵守原始证据→源解析→语义/校验→标准事实→财务派生→模型的方向。已有标准字段首先消费标准查询值，PDF 独立核验，不静默替代 float32 舍入值。确无已审核映射的细项可以作为显式补充输入，须区分“未审核”与“源不存在”；标准字段缺失/冲突不得自动回退 PDF。公司特定分析政策不写成全市场标准事实。

双公司 [valuation-chain 样本](../../internal/ingest/testdata/valuation-chain-2026/README.md)实际执行生产归档、解析、物化和查询，再交付估值输入。旧 PDF 模型是研究对照，不能单独证明生产链路完成；输入覆盖率按标准供应与补充项分别统计。
