...
部分行情服务器的 `GetReportFile` 返回空内容且不报错。该路径现在回退到 TDX 官方分发服务 `http://down.tdx.com.cn:8001/tdxfin/`，限制响应大小与下载时间；财务包仍必须匹配清单中的大小和 MD5，失败不落事实、不推进检查点。该 HTTP 分发服务与原行情协议均不提供传输层加密，MD5 用于核对上游文件完整性，不代表来源认证。

清单可能提前列出未来报告期的空占位包。选择最新 N 个包之前，先排除报告期晚于本次观察日（中国日期）的条目；`--all` 同样只处理截至观察日的报告期。实测清单中的 `gpcw20260930.zip` 在 9 月 5 日仅有 164 字节、零家公司，不能作为最新可用财报。
