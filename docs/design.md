...
多市场身份与 WACC 数据的后续表结构以 [ADR 016](decisions/016-multi-market-wacc-data.md) 为目标补充：拆分上市报价身份、增加不可变参考数据发布版本，并固定估值输入。迁移 027 已建立发布版本及四类参考观测表；身份扩展与同步／估值查询仍待实现，不改变当前 A 股生产范围。
