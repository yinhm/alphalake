...
多市场身份与 WACC 数据的后续表结构以 [ADR 016](decisions/016-multi-market-wacc-data.md) 为目标补充：拆分上市报价身份、增加不可变参考数据发布版本，并固定估值输入。迁移 027 已建立发布版本及四类参考观测表；国家风险、全球行业 Beta 和人民币国债曲线已接入限定同步链，身份扩展与参考数据估值查询仍待实现，不改变当前 A 股生产范围。
...
### 估值输入的消费边界（2026-09-06）

TDX 主要结构化数值、CNINFO 公告身份/时点/原文核验的分工不因公司样本研究改变。财务派生和估值优先消费标准事实查询，缺少已审核映射的附注输入明确标记补充；不得把 PDF 直接取值或解析器自洽误报为生产链路完成。源冲突不相互覆盖，精度差异与缺口保留。具体译本规则及双公司链路见 ADR 007 补充。
