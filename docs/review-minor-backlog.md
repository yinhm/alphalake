...
估值失败诊断仍需补齐：Python入口捕获子进程异常后只返回退出码说明，未展开底层stderr。此前查询默认读写ATTACH造成的锁冲突已完成[只读连接及两路主库并发验收](acceptance/readonly-valuation-20260911.json)，不再列为待修复项。剩余关闭标准：参考选择、就绪度和逐公司导出失败的底层原因在既有结构化失败中可见，保持原有失败隔离与退出码。
