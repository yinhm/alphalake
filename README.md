...
官方公司行业名单现可用 `alphalake sync-company-industries DB --python /absolute/valuation-python` 归档和发布来源观察，支持 `--offline` 重放；尚未自动写入本地证券分类或分配估值政策，范围和边界见[自动估值主线](docs/automated-valuation.md)。
